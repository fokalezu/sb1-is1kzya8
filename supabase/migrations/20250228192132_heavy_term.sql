...
BEGIN
  ...
  IF v_promo_code.expires_at IS NOT NULL AND v_promo_code.expires_at < now() THEN
    RETURN jsonb_build_object('success', false, 'message', 'Promo code has expired');
  END IF;
  
  -- Check if promo code has reached max uses
  IF v_promo_code.max_uses IS NOT NULL AND v_promo_code.current_uses >= v_promo_code.max_uses THEN
    RETURN jsonb_build_object('success', false, 'message', 'Promo code has reached maximum uses');
  END IF;
  ...
  EXCEPTION
    WHEN others THEN
      -- Return error
      RETURN jsonb_build_object('success', false, 'message', 'Error redeeming promo code: ' || SQLERRM);
  END;
END;
